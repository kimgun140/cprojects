...
void show_RoomList(std::string line, std::vector<std::string> &vec);
...
int main(int argc, char *argv[])
{
    ...
    serv_addr.sin_addr.s_addr = inet_addr("10.10.21.103");

    // serv_addr.sin_port = htons(atoi(argv[2]));
    serv_addr.sin_port = htons(atoi("10000"));
...
{
    std::string ending = nick + "님이 채팅방을 떠났습니다.\n";
    int sock = *((int *)arg);
    char msg[1024];
    int str_len;
    while (1)
...
void *send_msg(void *arg)
{
    ...
    std::string nickMsg;
    std::string ending = nick + "님이 채팅방을 떠났습니다.\n";
    // std::string starting = nick + "님이 채팅방에 들어왔습니다.\n";
    // write(sock, starting.c_str(), strlen(starting.c_str()));
    ...
    while (1)
    {
        ...
        if (msg == "q" || msg == "Q")
        {
            ...
            write(sock, ending.c_str(), strlen(ending.c_str()));
            return NULL;
        }
        nickMsg = nick + ": " + msg;
        write(sock, nickMsg.c_str(), strlen(nickMsg.c_str()));
        nickMsg = "";
        msg = "";
    }
    return NULL;
...
void *generate_clnt(void *arg)
{
    ...
    while (1)
    {
        ...
        else if (first == "1" || first == "로그인")
        {
            ...
            {
                std::string user_info = arriveMsg(sock);
                if (user_info != "e")
                {
                    std::stringstream user_info_stream(user_info);

                    getline(user_info_stream, no, ',');
                    getline(user_info_stream, id, ',');
                    getline(user_info_stream, nick, ',');
                }
                ...
                while (1)
                {
                    ...
                    {
                        res = "";
                        std::cout << "생성할 채팅방의 이름을 입력하시오.\n";
                        std::string room;
                        getline(std::cin, room);
                        deliverMsg(sock, room);
                        std::cout << room << "을 생성하였습니다.\n";
                        continue;
                    }
                    // 채팅창접속
                    else if (res == "2")
                    {
                        ...
                        while (1)
                        {
                            ...
                            std::cout << "채팅방 접속\n"
                                      ...
                                      << "1. 친구와 1:1 채팅방 생성하기\n"
                                      << "2. 원하는 채팅방에 접속하기\n";
                            std::cout << "원하시는 채팅 방법을 선택하시오.\n";
                            std::string chat_choice;
                            getline(std::cin, chat_choice);
                            deliverMsg(sock, chat_choice);
                            if (chat_choice == "q" || chat_choice == "Q")
                            {
                                break;
                            }
                            // 1:1
                            else if (chat_choice == "1")
                            {
                                /*친구목록보여주기*/
                                system("clear");
                                ...
                                /*친구선택*/
                                std::string chat_no = "";
                                auto it_d2 = mates.begin();
                                auto it_d1 = std::find(it_d2->begin(), it_d2->end(), chat_no);
                                do
                                {
                                    std::cout << "\n채팅을 원하는 친구의 고유번호를 입력하세요\n";
                                    getline(std::cin, chat_no);
                                    for (it_d2; it_d2 != mates.end(); it_d2++)
                                    {
                                        it_d1 = std::find(it_d2->begin(), it_d2->end(), chat_no);
                                    }
                                } while (it_d1 == it_d2->end());

                                /*친구와 채팅방 생성*/
                                deliverMsg(sock, chat_no);
                                
                                /*채팅방 퇴장*/
                                std::cout << chat_no << "와의 채팅방을 생성했습니다.\n";
                                std::cout << "\n계속하려면 ENTER\n";
                            ...
                            {
                                chatName = "";
                                std::vector<std::string> vec_room;
                                /*/ 채팅창 목록 출력*/
                                std::string roomList = arriveMsg(sock);
                                show_RoomList(roomList, vec_room);
                                auto it = std::find(vec_room.begin(), vec_room.end(), chatName);
                                do
                                {
                                    std::cout << "\n접속할 채팅방의 이름을 입력하시오.\n";
                                    getline(std::cin, chatName);
                                    it = std::find(vec_room.begin(), vec_room.end(), chatName);
                                } while (it == vec_room.end());
                                /* 채팅창 선택*/
                                // 채팅창 선택 송신
                                write(sock, chatName.c_str(), strlen(chatName.c_str()));
                                system("clear");
                                std::cout << chatName << " 채팅방에 접속합니다.\n잠시만 기다려주십시오.\n";
                                void *thread_return1;
                                sleep(2);
                                std::string starting = nick + "님이 채팅방에 들어왔습니다.\n";
                                std::cout << "(끝내려면 Q를 입력하시오.)\n";
                                write(sock, starting.c_str(), strlen(starting.c_str()));
                                pthread_create(&snd_thread, NULL, send_msg, (void *)&sock);
                                pthread_create(&rcv_thread, NULL, recv_msg, (void *)&sock);
                                pthread_join(snd_thread, &thread_return1);
                                pthread_join(rcv_thread, &thread_return1);
                                std::cout << "채팅방에서 나갑니다.\n";
                                std::cout << "\n계속하려면 ENTER\n";
